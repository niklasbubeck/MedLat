...
class MAR(nn.Module):
    ...
    def sample_orders(self, bsz):
        orders = torch.argsort(torch.rand(bsz, self.seq_len), dim=1).long()
        return orders
    ...
    def forward(self, imgs, cond_list):
        ...
        # mask tokens
        orders = self.sample_orders(bsz=patches.size(0)).to(patches.device)
        if self.training:
        ...
        if self.guiding_pixel:
            ...
            guiding_pixel_loss = torch.Tensor([0]).to(imgs.device).mean()
    ...
    def sample(self, cond_list, num_iter, cfg, cfg_schedule, temperature, filter_threshold, next_level_sample_function):
        ...
        # init token mask
        _device = cond_list[0].device
        mask = torch.ones(bsz, self.seq_len, device=_device)
        patches = torch.zeros(bsz, self.seq_len, 3 * self.patch_size**2, device=_device)
        orders = self.sample_orders(bsz).to(_device)
        ...
        for step in range(num_iter):
            ...
            mask_len = torch.Tensor([np.floor(self.seq_len * mask_ratio)]).to(_device)

            # masks out at least one for the next iteration
            mask_len = torch.maximum(torch.Tensor([1]).to(_device),
