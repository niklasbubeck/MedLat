...
'''
This version of the VectorQuantizer class comes from the Taming Transformers repository.
...
class VectorQuantizer(nn.Module):
    ...
    def get_codebook_entry(self, indices, shape=None):
        ...
        indices = indices.int() # ensure indices are integers
        # get quantized latent vectors
        z_q = self.embedding(indices)

        if shape is not None:
            z_q = z_q.view(shape)  # b, h , w, c
            # reshape back to match original input shape
            z_q = z_q.permute(0, 3, 1, 2).contiguous() # b, c, h ,w
        else: 
            z_q = z_q.view(-1, *self.z_q_shape)
...
class VQModel(nn.Module):
    def __init__(self,
                 ddconfig,
                #  lossconfig,       # TODO: Off for now to not download the LPIPS model
                 n_embed,
                 embed_dim,
                 ignore_keys=[],
                 image_key="image",
        ...
        # self.loss = instantiate_from_config(lossconfig) # TODO: Off for now to not download the LPIPS model
    ...
    def decode_code(self, code_b, out_shape=None):
        quant_b = self.quantize.get_codebook_entry(code_b, shape=out_shape)
